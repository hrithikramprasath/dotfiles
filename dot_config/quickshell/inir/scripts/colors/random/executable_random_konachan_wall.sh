...
page=$((1 + RANDOM % 200))
response=$(curl -fsSL --max-time 15 "https://konachan.net/post.json?tags=rating%3Asafe&limit=1&page=$page" 2>/dev/null || true)
if [ -z "$response" ] || ! echo "$response" | jq empty 2>/dev/null || [ "$(echo "$response" | jq 'length' 2>/dev/null || echo 0)" -eq 0 ]; then
    # Fallback to yande.re if Konachan is unreachable or empty
    response=$(curl -fsSL --max-time 15 "https://yande.re/post.json?tags=rating%3Asafe&limit=1&page=$page" 2>/dev/null || true)
fi

link=$(echo "$response" | jq -r '.[0].file_url // empty' 2>/dev/null || true)
if [ -z "$link" ] || [ "$link" = "null" ]; then
    echo "Failed to retrieve valid wallpaper URL from anime provider." >&2
    exit 1
fi

ext=$(echo "$link" | awk -F. '{print $NF}' | cut -d'?' -f1)
[ -z "$ext" ] && ext="jpg"
...
if ! curl -fsSL --max-time 30 "$link" -o "$tempDownload" 2>/dev/null; then
    echo "Failed to download wallpaper from $link" >&2
    rm -f "$tempDownload"
    exit 1
fi
