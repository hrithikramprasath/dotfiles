...
Singleton {
    ...
    FileView {
        ...
        // loaded() fires after every successful read, including reloads.
        // The loaded property may stay true while a watched file is refreshed.
        onLoaded: root._applyParsed(startupFileView.text())
        onLoadFailed: {
            root.ready = true
            root.status = "missing"
            root.entries = []
