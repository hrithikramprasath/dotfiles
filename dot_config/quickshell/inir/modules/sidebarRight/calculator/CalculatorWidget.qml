...
FocusScope {
    ...
    function calculate() {
        try {
            ...
            // Preserve exponent notation produced by scientific functions and
            // previous results. Reject invalid input instead of changing its value.
            const number = "(?:\\d+(?:\\.\\d*)?|\\.\\d+)(?:[eE][+-]?\\d+)?"
            const arithmetic = new RegExp("^\\s*[+-]?" + number
                + "(?:\\s*[+*/-]\\s*[+-]?" + number + ")*\\s*$")
            if (!arithmetic.test(finalExpr)) throw new Error("Invalid expression")
