...
Item { // Bar content region
    ...
    FocusedScrollMouseArea { // Left side | scroll to change brightness
        ...
        // Extend up to the left pill's inner edge but never exceed it so the
        // active window and sidebar button never collide into the center cluster.
        readonly property real maxAvailableLeftWidth: middleSection.leftPillX > 0
            ? Math.max(0, middleSection.leftPillX - (root.isIslands ? 8 : 4))
            : root.width
        width: root.layoutCompressionActive
            ? root.compressedLeftEdgeWidth
            : Math.min(maxAvailableLeftWidth, Math.max(implicitWidth, 0))
        ...
        EdgeIsland {
            ...
            width: root.layoutCompressionActive
                ...
                : Math.min(Math.max(0, parent.width - x), leftSectionRowLayout.implicitWidth + root.islandPad * 2)
        ...
        RowLayout {
            ...
            readonly property real availableContentWidth: Math.max(0, parent.width - anchors.leftMargin - anchors.rightMargin)
            width: root.isIslands && !root.layoutCompressionActive
                ? Math.min(implicitWidth, availableContentWidth)
    ...
    FocusedScrollMouseArea { // Right side | scroll to change volume
        ...
        readonly property real maxAvailableRightWidth: middleSection.rightPillEndX > 0
            ? Math.max(0, root.width - middleSection.rightPillEndX - (root.isIslands ? 8 : 4))
            : root.width
        width: root.layoutCompressionActive
            ? root.compressedRightEdgeWidth
            : Math.min(maxAvailableRightWidth, Math.max(implicitWidth, 0))
        ...
        EdgeIsland {
            ...
            x: root.layoutCompressionActive ? 0
                : Math.max(0, parent.width - rightSectionRowLayout.anchors.rightMargin - rightSectionRowLayout.implicitWidth - root.islandPad)
            width: root.layoutCompressionActive
                ? Math.max(0, parent.width - root.islandOuterInset)
                : Math.min(parent.width, rightSectionRowLayout.implicitWidth + root.islandPad * 2)
            height: Appearance.sizes.baseBarHeight - root.islandInset * 2
        }
