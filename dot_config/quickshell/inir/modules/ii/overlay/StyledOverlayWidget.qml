...
AbstractOverlayWidget {
    ...
    // Positioning & sizing — clamp inside parent screen bounds so widgets never spawn off-screen
    x: Math.round(root.parent && root.parent.width > 0 && root.width > 0
        ? Math.min(Math.max(0, persistentStateEntry.x), Math.max(0, root.parent.width - root.width))
        : persistentStateEntry.x)
    y: Math.round(root.parent && root.parent.height > 0 && root.height > 0
        ? Math.min(Math.max(0, persistentStateEntry.y), Math.max(0, root.parent.height - root.height))
        : persistentStateEntry.y)
    ...
    // Self-registration with OverlayContext & boundary verification
    Component.onCompleted: {
        reportPinnedState();
        reportClickableState();
        Qt.callLater(() => {
            if (root.parent && root.parent.width > 0 && root.width > 0) {
                const maxX = Math.max(0, root.parent.width - root.width);
                const maxY = Math.max(0, root.parent.height - root.height);
                if (persistentStateEntry.x > maxX || persistentStateEntry.y > maxY || persistentStateEntry.x < 0 || persistentStateEntry.y < 0) {
                    root.savePosition(root.x, root.y);
                }
            }
        });
    }
